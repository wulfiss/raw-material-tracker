<script lang="ts">
  ...
  function mapToIngredientRows(ings: typeof data.ingredients, mats: typeof materials) {
    return ings.map(ing => {
      const m = mats.find((x) => x.id === ing.material_id);
      return ({
        uid: crypto.randomUUID(),
        material_id: ing.material_id,
        quantity: String(ing.quantity),
        unit: ing.unit,
        lossPercent: ing.lossPercent !== null ? String(ing.lossPercent) : '',
        notes: ing.notes ?? '',
        searchQuery: m ? `${m.name} · ${m.category} · ${m.unit}` : '',
        searchFocused: false
      });
    });
  }
  ...
  $effect(() => {
    ingredients = mapToIngredientRows(data.ingredients, materials);
  });

  function makeSearchQuery(matId: string) {
    const m = materials.find((x) => x.id === matId);
    return m ? `${m.name} · ${m.category} · ${m.unit}` : '';
  }
  ...
  const f = $derived(form?.fields ?? data.recipe);
</script>

<div class="mb-8">
  <p class="text-xs font-bold uppercase tracking-[0.18em] text-primary">{$t.recipes.subtitle}</p>
  <h1 class="mt-2 text-3xl font-bold tracking-tight">{$t.common.editRecipe}</h1>
</div>
  ...
  <Card class="mb-6">
    ...
    <CardContent class="grid gap-5 md:grid-cols-2">
      <div class="grid gap-2 md:col-span-2">
        ...
        <Input id="name" name="name" required value={f?.name ?? ''} placeholder={$t.newRecipe.fields.namePlaceholder} />
      </div>
      <div class="grid gap-2">
        <Label for="category">{$t.newRecipe.fields.category}</Label>
        <Input id="category" name="category" value={f?.category ?? ''} placeholder={$t.newRecipe.fields.categoryPlaceholder} />
      </div>
      <div class="grid gap-2">
        <Label for="yieldQuantity">{$t.newRecipe.fields.yieldQuantity}</Label>
        <Input id="yieldQuantity" name="yieldQuantity" type="number" min="0.001" step="0.001" required value={f?.yieldQuantity ?? ''} />
      </div>
      <div class="grid gap-2">
        <Label for="yieldUnit">{$t.newRecipe.fields.yieldUnit}</Label>
        <Select id="yieldUnit" name="yieldUnit" required value={f?.yieldUnit ?? 'kg'}>
      ...
      <div class="grid gap-2 flex flex-row items-center gap-4 md:col-span-2">
        <div class="flex items-center space-x-2">
          <input type="checkbox" id="active" name="active" checked={f?.active ?? true} class="h-4 w-4" />
